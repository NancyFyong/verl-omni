...
See [the simplex OPD recipe](../../examples/opd_trainer/minicpm_o/README.md) for data conversion, checkpoint compatibility, and launch settings. Video, speech-policy training, and streaming duplex are not supported by the simplex recipe. A separate [experimental bounded duplex path](../../examples/opd_trainer/minicpm_o/README.md#experimental-bounded-native-duplex) adds native input/action trace and causal teacher replay; live cancellation and real-weight multi-step validation remain outstanding.
