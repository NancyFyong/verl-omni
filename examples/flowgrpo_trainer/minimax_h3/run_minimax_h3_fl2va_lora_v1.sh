#!/usr/bin/env bash
# MiniMax H3 FL2VA (first-frame conditioned) LoRA FlowGRPO (V1 trainer: TransferQueue + ReplayBuffer + sync mode).
#
# This is the V1 sync counterpart of run_minimax_h3_fl2va_lora.sh. It uses
# `verl_omni.trainer.main_diffusion_v1` with `trainer.use_v1=true` and
# `trainer.v1.trainer_mode=sync`. TransferQueue is enabled by the runner;
# model, LoRA, reward, pipeline, and CPS FlowGRPO knobs match the V0 recipe.
set -x
...
python3 -m verl_omni.trainer.main_diffusion_v1 \
    ...
    trainer.experiment_name=minimax_h3_fl2va_lora_v1_gpu \
