...
#
# Rollout side: unchanged vllm_omni in-tree LingBot transformer (dense+MoE,
# PR #5035); weight sync streams `transformer.<fqn>` 1:1 (experts EP-restored
# to full [E,...] tensors, router/norms kept fp32).
#
# Mirrors run_lingbot_dense_t2v_lora.sh for the pipeline/algo knobs and
# run_qwen_image_ocr_veomni.sh for the engine-selection overrides.
# Preprocess structured captions first; the rollout agent rejects plain-text
# prompts by design.
set -euo pipefail
