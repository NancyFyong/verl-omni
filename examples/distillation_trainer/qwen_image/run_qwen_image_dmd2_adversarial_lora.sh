#!/usr/bin/env bash
# Qwen-Image four-step DMD2 with separate fake-score and discriminator optimizers.
set -xeuo pipefail

WORKSPACE=${WORKSPACE:-$HOME}
MODEL_PATH=${MODEL_PATH:-Qwen/Qwen-Image}
TRAIN_FILES=${TRAIN_FILES:-${WORKSPACE}/data/qwen_image_dmd2_adversarial/train.parquet}
VAL_FILES=${VAL_FILES:-${WORKSPACE}/data/qwen_image_dmd2_adversarial/test.parquet}
NUM_GPUS=${NUM_GPUS:-8}
...
python3 -m verl_omni.trainer.main_diffusion \
    ...
    distillation.distribution_matching.discriminator_optim.weight_decay=0.001 \
    ...
    distillation.distribution_matching.adversarial.generator_weight=5e-3 \
    distillation.distribution_matching.adversarial.discriminator_weight=1e-2 \
    ...
    trainer.experiment_name=qwen-image-dmd2-adversarial-lora \
