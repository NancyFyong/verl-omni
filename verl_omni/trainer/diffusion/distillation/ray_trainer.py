...
class DistillationRayTrainer(BaseRayDiffusionTrainer):
    ...
    def _save_checkpoint(self) -> None:
        ...
        try:
            ...
            torch.save(self.controller.state_dict(), os.path.join(temporary_path, "trainer_state.pt"))
    ...
    def _load_checkpoint(self) -> int:
        ...
        self.controller.load_state_dict(trainer_state)
        ...
        self.global_steps = self.controller.counters.global_step
    ...
    def fit(self, num_cycles: Optional[int] = None) -> None:
        ...
        target_steps = (
            ...
            if num_cycles is None
            else min(self.total_training_steps, controller.counters.global_step + num_cycles)
        )
        progress_bar = tqdm(
            total=target_steps,
            initial=controller.counters.global_step,
            desc="Distillation Training",
        )
        profile_steps = OmegaConf.select(self.config, "global_profiler.steps", default=None)
        while controller.counters.global_step < target_steps:
            before_global_step = controller.counters.global_step
            profile_step = before_global_step + 1
            do_profile = (
                profile_steps is not None
                and profile_step in profile_steps
                and controller.counters.completed_cycles >= self.plan.update_schedule.warmup_cycles
            )
            if do_profile:
            ...
            self.global_steps = controller.counters.global_step
            metrics = self.flatten_metrics(controller.metrics)
            ...
            metrics.update(
                {
                    ...
                    for role, steps in controller.counters.optimizer_steps.items()
                }
            )
            metrics["training/global_step"] = float(self.global_steps)
            metrics["training/completed_cycles"] = float(controller.counters.completed_cycles)
            self._logger.log(data=metrics, step=controller.counters.completed_cycles)
