...
class MiniMaxH3DiffusionSingleTurnAgentLoop(DiffusionSingleTurnAgentLoop):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Deliberately not calling ``super().__init__``: the upstream base runs
        # ``initialize_system_prompt``, which mutates the shared Rust tokenizer
        # through ``no_truncation()``. ``DiffusionAgentLoopWorker`` instantiates
        # agent loops concurrently under ``asyncio.gather``, so that mutation
        # races and aborts the rollout with "RuntimeError: Already borrowed".
        # H3 tokenizes prompt text verbatim and never applies a chat template,
        # so there is no system prompt to derive here.
        del kwargs
