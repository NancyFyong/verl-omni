...
def _minicpmo_forward(
    ...
    **kwargs,
    ...
    duplex_replay = kwargs.pop("minicpm_duplex_replay", None)
    if duplex_replay is not None:
        from .duplex_training import forward_duplex

        kwargs.pop("image_bound", None)
        return forward_duplex(self, input_ids, attention_mask, position_ids, duplex_replay, **kwargs)
    if input_ids.ndim != 2:
...
class MiniCPMThinkerAdapter(OmniModelBase):
    ...
    def prepare_model_inputs(cls, model_inputs, micro_batch, model_config):
        ...
        if "minicpm_duplex_replay" in model_inputs:
            if model_config.lora_rank <= 0 or set(model_config.target_modules) - {
                "q_proj",
                "k_proj",
                "v_proj",
                "o_proj",
                ...
                raise ValueError(
                    "Duplex replay currently requires attention-only LoRA with frozen embeddings/encoders."
                )
        return model_inputs
