...
class PPODiffusersFSDPEngine(DiffusersFSDPEngine):
    ...
    def prepare_model_outputs(self, output, micro_batch: TensorDict):
        ...
        # value ``coe`` (the temporal-gradient-rectification weight); pipelines
        # that do not still return the 4-tuple, so ``rest`` is empty and ``coe``
        # stays absent (FlashGRPOLoss then degenerates to FlowGRPO).
