...
def test_configure_train_mode_rejects_gradient_checkpointing_models():
    ok = torch.nn.Linear(2, 2)
    ok._supports_gradient_checkpointing = False
    LingBotVideoDenseFlowGRPO.configure_train_mode(ok)  # must not raise
    bad = torch.nn.Linear(2, 2)
    bad._supports_gradient_checkpointing = True
    with pytest.raises(RuntimeError, match="gradient checkpointing"):
        LingBotVideoDenseFlowGRPO.configure_train_mode(bad)
