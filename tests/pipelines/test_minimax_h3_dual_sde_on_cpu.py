...
"""CPU tests for the MiniMax H3 dual-stream SDE scheduler."""
...
class TestMiniMaxH3TimeShiftSigmas:
    def test_descends_from_one_to_zero(self):
        sigmas = flow_match_shift_sigmas(_NUM_STEPS, _VIDEO_SHIFT)
    ...
    def test_larger_shift_pushes_mass_toward_high_noise(self):
        video = flow_match_shift_sigmas(_NUM_STEPS, _VIDEO_SHIFT)
        audio = flow_match_shift_sigmas(_NUM_STEPS, _AUDIO_SHIFT)
        assert video[1] > audio[1]

    def test_rejects_nonpositive_arguments(self):
        with pytest.raises(ValueError):
            flow_match_shift_sigmas(_NUM_STEPS, 0.0)
        with pytest.raises(ValueError):
            flow_match_shift_sigmas(0, _VIDEO_SHIFT)
...
class TestDualSchedulerSigmaAlignment:
    def test_per_stream_sigmas_match_helper_no_double_shift(self):
        ...
        assert video == [round(s, 5) for s in flow_match_shift_sigmas(_NUM_STEPS, _VIDEO_SHIFT)]
        assert audio == [round(s, 5) for s in flow_match_shift_sigmas(_NUM_STEPS, _AUDIO_SHIFT)]
