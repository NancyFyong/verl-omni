...
"""Local SP allocation with the unmodified verl manager/replica/IPC lifecycle."""
...
async def test_manager_replica_and_worker_slices(monkeypatch, tp, usp, ring, standalone):
    from verl_omni.workers.rollout import llm_server
    ...
    manager = llm_server.DiffusionLLMServerManager(
...
def test_registered_adapter_rank_and_ipc_mapping(monkeypatch, tp, usp, ring, explicit_replica):
    ...
    from verl_omni.workers.rollout.vllm_rollout.vllm_omni_adapter import vLLMOmniServerAdapter
...
async def test_sp_one_manager_delegates(monkeypatch):
    ...
    from verl_omni.workers.rollout.llm_server import DiffusionLLMServerManager
...
async def test_manager_start_rank_and_metrics(monkeypatch):
    ...
    manager = llm_server.DiffusionLLMServerManager(cfg, start_rank=3)
